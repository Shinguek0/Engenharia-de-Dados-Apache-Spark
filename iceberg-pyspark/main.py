...
def create_my_table(spark: SparkSession):
    spark.sql("""
      CREATE TABLE IF NOT EXISTS my_iceberg_catalog.db.babynames (
        genero string,
        etinia string,
        name string,
        rank bigint
      ) USING iceberg PARTITIONED BY (etinia, name)
    """)
...
def write_my_data(spark: SparkSession):
    current_dir = os.path.realpath(os.path.dirname(__file__))
    path = os.path.join(current_dir, "Popular_Baby_Names", "Popular_Baby_names.csv")
    ...
    vaccinations \
      .withColumn("etinia", F.col("Ethnicity")) \
      .writeTo("my_iceberg_catalog.db.babynames") \
      .append()
